...
def wrinkleDetection(imgLocation, fileName):
    ...
    i = 0
    print(type(faces))
    while(np.logical_and(faces.size == 0, i < 4)):
    ...
    #     adjusted_score = 0 + (score - 0) * (100 - 0) / (600 - 0)
    #     return int(100 - adjusted_score)


# def wrinkleDetection():
...
#         print("Hello")
#         os.chdir(directory)
#         cv2.imwrite("48-wd-something.jpg", img)
#         score = np.count_nonzero(under_left_eye_canny)
#         adjusted_score = 0 + (score - 0) * (100 - 0) / (600 - 0)
#         return int(100 - adjusted_score)
